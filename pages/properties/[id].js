...
import PropertyStats from '@/features/Property/components/PropertyThumbnailSlider/PropertyStats';
    ...
    return(
     ...
     <Box backgroundColor='#f7f8f9' paddingY='3rem' >
        <Grid templateColumns='repeat(6, 1fr)' gap='5' maxWidth='1280px' margin='0 auto'>
            <GridItem colSpan='6' >
                <Text fontSize='3xl' fontWeight='medium' color='blue.800' textAlign={{base: 'center', sm: 'left'}}>{propertyType} {title}</Text>
            ...
            <GridItem  colSpan={{base: 6, sm: 3}}>
                ...
                <SimpleGrid columns={{base:1, sm: 2}} fontWeight='light' color='gray.600' fontSize='1rem'>
                    {amenities.length ? amenities.map((item) => <Text>{item}</Text>) : 'Ətraflı məlumat üçün əlaqə saxlamağınız xahiş olunur'}
                </SimpleGrid>
                </TextContentBox>
            </GridItem>
            <GridItem colSpan={{base: 6, sm: 3}}>
